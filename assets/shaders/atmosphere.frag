...
const int POINTS_FROM_CAMERA = 10; // number sample points along camera ray
const int OPTICAL_DEPTH_POINTS = 10; // number sample points along light ray
...
// uniforms
layout(set = 1, binding = 0) uniform texture2D screenTexture; // the original screen texture
layout(set = 1, binding = 1) uniform sampler screenTextureSampler; // the original screen texture

layout(set = 1, binding = 2) uniform texture2D depthTexture; // the depth texture
layout(set = 1, binding = 3) uniform sampler depthTextureSampler; // the depth map of the camera

layout(set = 1, binding = 4) uniform AtmosphereSettings {
    vec3 sunPosition; // position of the sun in world space
    vec3 cameraPosition; // position of the camera in world space
    mat4 inverseProjection; // camera's inverse projection matrix
    mat4 inverseView; // camera's inverse view matrix
    float cameraNear; // camera minZ
    float cameraFar; // camera maxZ

    vec3 planetPosition; // planet position in world space
    float planetRadius; // planet radius for height calculations
    float atmosphereRadius; // atmosphere radius (calculate from planet center)

    float falloffFactor; // controls exponential opacity falloff
    float sunIntensity; // controls atmosphere overall brightness
    float scatteringStrength; // controls color dispersion
...
void main() {
    vec3 screenColor = texture(sampler2D(screenTexture, screenTextureSampler), v_Uv).rgb;

    float depth = texture(sampler2D(depthTexture, depthTextureSampler), v_Uv).r; // the depth corresponding to the pixel in the depth map
    ...
    vec3 deepestPoint = worldFromUV(v_Uv, depth) - cameraPosition;
